...
import { FiKey, FiSend, FiCheckCircle } from "react-icons/fi";
...
const ChangePassword = () => {
    ...
    const sendOtp = async () => {
        ...
        try {
            const { data } = await api.post("/users/send-reset-password-otp", { email });
    ...
    const change = async (e) => {
        ...
        setMessage({ type: "", text: "" });
        try {
            const { data } = await api.post("/users/verify-reset-password", {
                email,
                otp,
                newPassword,
            });
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-4">
            <div className="rounded-2xl border border-slate-200 bg-white p-5">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500">
                            We’ll send a 6-digit OTP to <span className="font-medium">{email}</span>.
                        </p>
                    </div>
                </div>
                ...
                {message.text && (
                    <div
                        className={`mt-4 rounded-xl border px-4 py-3 text-sm ${message.type === "error"
            ...
            <form onSubmit={change} className="rounded-2xl border border-slate-200 bg-white p-5 space-y-3">
                <div>
                ...
                <button
                    type="submit"
                    disabled={verifying || !otp.trim() || !newPassword.trim() || !email}
